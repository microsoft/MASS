...
from . import FairseqMultiModel, register_model, register_model_architecture, BaseFairseqModel
...
class XTransformerDecoder(TransformerDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, final_norm=True):
        super().__init__(args, dictionary, embed_tokens, no_encoder_attn, final_norm)
...
@register_model('xencoder')
class XEncoder(BaseFairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        embed_token = build_embedding(
            task.dictionary, args.encoder_embed_dim, args.encoder_embed_path        
        )

        encoder = XTransformerEncoder(args, task.dictionary, embed_token)
        return cls(args, encoder)

    def forward(self, tokens, segment_labels):
        return self.encoder._forward(tokens)
...
class XTransformerModel(BaseFairseqModel):
    ...
    def add_args(parser):
        TransformerModel.add_args(parser)
        parser.add_argument('--share-lang-embeddings', action='store_true',
                            help='Share embeddings in single language')
        parser.add_argument('--share-encoders', action='store_true',
                            help='share encoders across languages')
        parser.add_argument('--share-decoders', action='store_true',
                            help='share decoders across languages')
        parser.add_argument('--load-encoder-weights', default="", type=str,
                            help='Load Encoder weights')
...
@register_model_architecture('xencoder', 'xencoder_lm')
def base_x_encoder(args):
    base_architecture(args)
