    ...
    def generate(
        ...
        **kwargs
        ...
        encoder_input = {
            ...
            if k != 'prev_output_tokens' and k != 'segment_label'
        }

        tgt_segment_label = sample['net_input']['segment_label'] \
                            if 'segment_label' in sample['net_input'].keys() else None
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = model.forward_decoder(tokens[:, :step + 1], encoder_outs, tgt_segment_label=tgt_segment_label)
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, tgt_segment_label=None):
        if len(self.models) == 1:
            return self._decode_one(
                ...
                log_probs=True,
                tgt_segment_label=tgt_segment_label,
            )

        log_probs = []
        avg_attn = None
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn = self._decode_one(tokens, model, encoder_out, self.incremental_states, log_probs=True, tgt_segment_label=tgt_segment_label)
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs, tgt_segment_label=None):
