...
class MaskedLanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        if self.training is False:
            source   = self.src[index].tolist()
            src_list = [self.vocab.eos_index] + self.src[index].tolist()
            target, output = source[:-1], source[1:]
        else:
            src_item = self.src[index]
            src_list = src_item.tolist()
            source, target, output, positions = [], [], [], []
            prev = self.vocab.eos_index
            for i, w in enumerate(src_list):
                p = np.random.random()
                if (i < len(src_list) - 1) and (p >= self.ratio):
                    source.append(self.mask_word(w))
                    output.append(w)
                    target.append(prev)
                    positions.append(i + self.vocab.pad_index + 1)
                else:
                    source.append(w)
                prev = w
            if len(target) == 0:
                target.append(self.vocab.eos_index)
                output.append(src_list[0])
                positions.append(self.vocab.pad_index + 1)
    ...
    def _collate(self, samples, pad_idx, eos_idx, segment_label):
        ...
        if 'positions' in samples[0].keys():
            positions = merge('positions', left_pad=self.left_pad_target)
            positions = positions.index_select(0, sort_order)
            batch['net_input']['positions'] = positions
        return batch
